...
export default function PixelSpace({en}:{en:boolean}) {
 ...
 useEffect(()=>{
  ...
  if(initialMotion.matches||paused||device.connection?.saveData)return;
  const deferred=window.setTimeout(()=>{void import('three').then(THREE=>{
   ...
   const constrained=(device.deviceMemory??8)<=4||navigator.hardwareConcurrency<=4;
   const positions=new Float32Array((constrained?120:window.innerWidth<700?220:500)*3);
   let seed=412;
   ...
   let frame=0,timer=0,last=0,elapsed=0,lost=false;
   const draw=()=>{if(!lost)renderer.render(scene,camera);};
   const stop=()=>{cancelAnimationFrame(frame);window.clearTimeout(timer);frame=0;};
   const tick=(now:number)=>{elapsed+=Math.min((now-last)/1000,.1);last=now;stars.rotation.z=Math.sin(elapsed*.025)*.018;stars.position.y=Math.sin(elapsed*.08)*.22;draw();timer=window.setTimeout(()=>{frame=requestAnimationFrame(tick);},constrained?90:50);};
   const sync=()=>{stop();last=performance.now();if(!document.hidden&&!paused&&!motion.matches&&!lost)frame=requestAnimationFrame(tick);else draw();};
   ...
   const contextLost=(event:Event)=>{event.preventDefault();lost=true;stop();};
