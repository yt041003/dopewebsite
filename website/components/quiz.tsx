...
import ResultNext from './result-next';
import SocialLinks from './social-links';
import {stableResultPath} from '@/lib/growth';
...
export default function Home({initialLocale,children,navigation}:{initialLocale:'en'|'zh-hant';children?:React.ReactNode;navigation?:React.ReactNode}) {
 ...
 function switchLanguage(event:React.MouseEvent<HTMLAnchorElement>,locale:string){
  ...
  window.location.assign('/'+locale+'/dope');event.preventDefault();
 }
 useEffect(()=>{
 ...
 async function shareResult(copyOnly=false){
  const valuesForLink=score(answers),maxForLink=Math.max(...valuesForLink),leadForLink=birds.filter((_,i)=>valuesForLink[i]===maxForLink);
  const url=window.location.origin+stableResultPath(initialLocale,'dope',leadForLink.length===1?leadForLink[0].code:undefined);
