...
import {quizForContent} from '@/lib/growth';
...
export default function EditorialPage({locale,page}:{locale:Locale;page:ContentPage}){
 const en=locale==='en',i=en?1:0,quiz=quizForContent(page.path);
 ...
 return <div className="explore-theme editorial-theme" style={theme}><PixelSpace en={en}/><div className="site-shell">
  ...
  <main id="main-content" className="editorial-main"><Breadcrumbs locale={locale} path={page.path}/><article>
   ...
   {page.kind==='guide'&&<p className="byline">{en?'By ':'作者：'}<a href={`/${locale}/about`}>Twynzo</a>{en?' · Original self-reflection guide':' · 原創自我探索指南'}</p>}
   ...
   {page.kind==='type'&&<p className="editorial-disclaimer">{en?'Read this as a set of possibilities to compare with your experience, not a complete description of you. These are original reflection prompts, not validated assessments.':'把這篇當作可以與經驗比較的可能性，而不是對你的完整定義。以下為原創反思提示，並非經驗證的評估。'}{page.path.startsWith('/personality/')&&(en?' These are not official MBTI® profiles.':' 本文並非官方 MBTI® 類型解讀。')}</p>}
   {quiz&&!page.sections.some(section=>section.worksheet)&&<QuizCta locale={locale} kind={quiz}/>}
   ...
   {page.sections.map((section,n)=><section id={'section-'+n} key={n}><h2>{section.heading[i]}</h2><p>{section.body[i]}</p>{section.items&&(section.ordered?<ol className="article-steps">{section.items.map((item,k)=><li key={k}>{item[i]}</li>)}</ol>:<ul className="article-steps">{section.items.map((item,k)=><li key={k}>{item[i]}</li>)}</ul>)}{section.table&&<div className="article-table-scroll"><table><caption>{section.table.caption[i]}</caption><thead><tr>{section.table.headers.map((head,k)=><th key={k} scope="col">{head[i]}</th>)}</tr></thead><tbody>{section.table.rows.map((row,k)=><tr key={k}>{row.map((cell,j)=>j===0?<th key={j} scope="row">{cell[i]}</th>:<td key={j}>{cell[i]}</td>)}</tr>)}</tbody></table></div>}{section.worksheet&&<pre className="article-worksheet">{section.worksheet[i]}</pre>}</section>)}
   ...
   {page.sources&&<section><h2>{en?'Concept sources':'概念來源'}</h2><ul className="article-steps">{page.sources.map(source=><li key={source.url}><a href={source.url}>{source.label[i]}</a></li>)}</ul><p>{en?'These sources explain the named concepts; they do not validate or endorse Twynzo’s original questions or exercises.':'以上來源說明相關概念，並不代表其認證或支持 Twynzo 的原創題目與練習。'}</p></section>}
   <MonetizationSlot placement="article-break"/>
   {quiz&&<QuizCta locale={locale} kind={quiz}/>}
