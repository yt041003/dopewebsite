...
export function exploreMetadata(locale:Locale,test?:typeof tests[number]):Metadata {
 ...
 const title=test?.slug==='personality-16'?(en?'Free 16-Type Personality Quiz, Inspired by MBTI | Twynzo':'16 型人格測驗｜MBTI 靈感的免費人格探索 — Twynzo'):test?test.name[i]+(en?' — Free quiz | Twynzo':'｜免費測驗 — Twynzo'):(en?'Twynzo | DOPE TEST, MBTI & Personality Exploration':'Twynzo｜免費 DOPE TEST 人格測試・MBTI 人格探索');
 const description=test?.slug==='personality-16'?(en?'Learn about MBTI’s four preference pairs, 16 personality types and how they differ from DOPE. Take Twynzo’s original 24-question quiz and download your result.':'認識 MBTI 四組偏好、16 型人格與 DOPE TEST 的差異，了解人格測試如何幫助自我探索。免費完成 24 道原創情境題，探索你的偏好並下載結果圖卡。'):test?test.description[i]:(en?'Explore personality, communication and relationships with Twynzo. Start the free bilingual DOPE quiz and discover more self-reflection tests.':'透過 Twynzo 探索人格、溝通與親密關係。立即體驗免費中英文 DOPE 鳥類性格測驗，發現更多認識自己的方式。');
 return {...metadataFor(locale),title,description,robots:{index:true,follow:true},alternates:{canonical:`/${locale}${suffix}`,languages:{'zh-Hant':`/zh-hant${suffix}`,en:`/en${suffix}`,'x-default':`/zh-hant${suffix}`}},openGraph:{type:'website',siteName:'Twynzo',title,description,url:`/${locale}${suffix}`,images:['/birds-pixel.png']},twitter:{card:'summary_large_image',title,description,images:['/birds-pixel.png']}};
}
